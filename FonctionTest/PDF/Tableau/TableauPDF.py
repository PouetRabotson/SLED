...
# Créer une page pdf test_report_lab, de taille A4
doc = SimpleDocTemplate("test_report_lab.pdf", pagesize = A4)
doc.pagesize = portrait(A4) # Une page A4 format portrait

tailleTexte = 8

styles = getSampleStyleSheet()
styleP = styles['Normal']
styleP.fontSize = tailleTexte

# Liste qui contiendra toutes les infos de notre tableau
element = []

# Contient les données à affiche de notre tableau
data = [['Nom', 'Date', 'Voc [V]', 'Isc [mA]', 'Pm [mW]', 'Ff [%]', 'Rp [Ohm]', 'Rs [Ohm]', 
			'Vm [V]', 'Im [mA]']]

# Pour toutes les mesures
for ma_mesure in listeMesures:

	# On créer une liste qui contient toutes les informations de notre mesure.
	# Noter que les floats sont arrondis a 2 chiffres après la virgule.
	# 	et que les nombres sont transformés en chaine de caractère.

	if (ma_mesure.rp >= 10000 or ma_mesure.rp <= -10000):
		rp = 'OoR' # Out of Range
	else:
		rp = str("%.3f" % ma_mesure.rp)
				
	if (ma_mesure.rs >= 10000 or ma_mesure.rs <= -10000):
		rs = 'OoR' # Out of Range
	else:
		rs = str("%.3f" % ma_mesure.rs)

	ligne = [[Paragraph(ma_mesure.nom, styleP), Paragraph(ma_mesure.date, styleP), 
				str("%.3f" % ma_mesure.voc), str("%.3f" % ma_mesure.isc), str("%.3f" % ma_mesure.pm), 
				str("%.3f" % ma_mesure.ff), rp, rs, str("%.3f" % ma_mesure.vm), str("%.3f" % ma_mesure.im)]]

	# On ajoute la ligne de la mesure dans les données du tableau
	data.extend(ligne)

# On crée la mise en forme du tableau
style = TableStyle([('FONTSIZE', (0,1), (-1,-1), tailleTexte),
...
colwidth = (5*cm, 3.2*cm, 1.5*cm, 1.5*cm, 1.5*cm, 1.5*cm, 1.5*cm, 1.5*cm, 1.5*cm, 1.5*cm)


# On créer l'objet Table avec toutes les informations nécessaires
